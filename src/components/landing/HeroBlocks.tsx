...
const typingPhrases = ["How does AI work?", "Explain quantum computing", "What is machine learning?", "Tell me about neural networks"];
export function HeroBlocks() {
  ...
  const AttachmentUI = ({
    ...
    showAttachment?: boolean;
      ...
      {showAttachment && <div className="flex items-center gap-2 px-2.5 py-1 bg-muted/40 rounded-lg border border-border/30">
  ...
  return (
    ...
    */
    <div className={cn("relative w-full h-full flex items-center justify-center", "origin-center", "motion-reduce:!transform-none", /* Desktop/laptop: no scaling (original size) */
    "lg:scale-100 xl:scale-100", /* Tablet: larger scale to fill frame */
    "md:scale-[0.65] max-lg:md:scale-[0.70]", /* Mobile: much larger scale for prominence, positioned lower to avoid header overlap */
    "max-md:scale-[0.75] max-sm:scale-[0.70]", /* Mobile: shift down slightly so animation doesn't overlap text/buttons at top */
    "max-md:translate-y-8")}>
      {/* SVG Connection Lines - Softer glow with smoother animation */}
      <svg className="absolute inset-0 w-full h-full pointer-events-none z-0 responsive-media" viewBox="0 0 500 700" preserveAspectRatio="xMidYMid meet">
        <defs>
          {/* Softer glow filter */}
        ...
        {/* Base dashed lines */}
        <path d="M 100 170 C 100 220, 200 240, 200 280" fill="none" stroke="hsl(var(--primary) / 0.1)" strokeWidth="2" strokeLinecap="round" strokeDasharray="8 6" />
        <path d="M 200 430 C 200 480, 120 500, 120 530" fill="none" stroke="hsl(var(--primary) / 0.1)" strokeWidth="2" strokeLinecap="round" strokeDasharray="8 6" />

        {/* Animated flowing lines with soft glow - hidden for reduced motion */}
        <path d="M 100 170 C 100 220, 200 240, 200 280" fill="none" stroke="url(#flowLine1)" strokeWidth="2.5" strokeLinecap="round" filter="url(#softGlow)" className="motion-reduce:hidden" />
        <path d="M 200 430 C 200 480, 120 500, 120 530" fill="none" stroke="url(#flowLine2)" strokeWidth="2.5" strokeLinecap="round" filter="url(#softGlow)" className="motion-reduce:hidden" />

        {/* Static fallback for reduced motion */}
        <path d="M 100 170 C 100 220, 200 240, 200 280" fill="none" stroke="hsl(var(--primary) / 0.2)" strokeWidth="2" strokeLinecap="round" className="hidden motion-reduce:block" />
        <path d="M 200 430 C 200 480, 120 500, 120 530" fill="none" stroke="hsl(var(--primary) / 0.2)" strokeWidth="2" strokeLinecap="round" className="hidden motion-reduce:block" />
      </svg>
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 sm:gap-12 md:gap-14 lg:gap-16">
        ...
        <div style={{
        ...
        }} className={cn("relative motion-reduce:!transform-none", "w-[20rem] sm:w-[22rem]", "rounded-2xl", "bg-gradient-to-br from-secondary/90 via-secondary/60 to-secondary/30", "border border-border/40", "shadow-[0_8px_32px_-8px_hsl(var(--primary)/0.15)]", "backdrop-blur-xl")}>
          {/* Header with dots */}
          ...
          <div className="flex items-center justify-between gap-2 px-4 pb-4 pt-2 border-t border-border/20">
            ...
            <button aria-label="Send" className="flex items-center justify-center w-10 h-10 rounded-full bg-foreground hover:bg-foreground/90 text-background transition-all duration-200 shadow-lg hover:shadow-xl hover:scale-105">
        ...
        }} className={cn("relative motion-reduce:!transform-none", "w-[22rem] sm:w-[24rem] md:w-[26rem]", "rounded-2xl", "bg-gradient-to-br from-secondary/90 via-secondary/60 to-secondary/30", "border border-border/40", "shadow-[0_8px_32px_-8px_hsl(var(--primary)/0.15)]", "backdrop-blur-xl")}>
          {/* Header with dots */}
          <div className="flex items-center justify-between px-6 pt-5 pb-2">
        ...
        }} className={cn("relative motion-reduce:!transform-none", "w-[18rem] sm:w-[20rem]", "rounded-2xl", "bg-gradient-to-br from-secondary/90 via-secondary/60 to-secondary/30", "border border-border/40", "shadow-[0_8px_32px_-8px_hsl(var(--primary)/0.15)]", "backdrop-blur-xl")}>
          {/* Header with dots */}
          <div className="flex items-center justify-between px-6 pt-5 pb-2">
            <p className="text-sm text-muted-foreground">AI Assistant</p>
            <MoreHorizontal className="w-5 h-5 text-muted-foreground/60" />
