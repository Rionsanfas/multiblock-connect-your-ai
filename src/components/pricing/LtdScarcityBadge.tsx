        ...
        <div className="relative bg-card/95 backdrop-blur-xl rounded-2xl px-6 py-4">
          ...
          <div className="relative h-2 bg-muted/50 rounded-full overflow-hidden mb-3">
            <div className={`
                ...
                ${soldOut ? 'bg-muted' : isCritical ? 'bg-gradient-to-r from-destructive to-destructive/70' : 'bg-gradient-to-r from-accent to-accent/70'}
              `} style={{
            width: `${100 - percentRemaining}%`
          }} />
            {!soldOut && <div className="absolute inset-y-0 right-0 animate-pulse" style={{
            left: `${100 - percentRemaining}%`,
            background: isCritical ? 'linear-gradient(90deg, transparent, hsl(var(--destructive) / 0.5))' : 'linear-gradient(90deg, transparent, hsl(var(--accent) / 0.5))'
          }} />}
          </div>

          {/* Count display */}
          <div className="flex items-center justify-center gap-2">
            {soldOut ? <span className="text-muted-foreground font-medium">All seats claimed</span> : <>
          ...
          {/* Urgency message */}
          {!soldOut && isCritical && <p className="text-center text-xs text-destructive mt-2 animate-pulse">
