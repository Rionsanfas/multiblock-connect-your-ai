...
export default function Auth() {
  ...
  const getSubtitle = () => {
    switch (mode) {
      case 'signup':
        return 'Start building with AI-powered blocks';
      case 'forgot-password':
  ...
  return <div className="min-h-screen flex items-center justify-center p-4 overflow-hidden relative bg-[hsl(220_20%_4%)] dot-grid-bg">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        {/* Left accent */}
        <div className="absolute top-1/2 left-0 -translate-y-1/2 w-[400px] h-[600px] opacity-20" style={{
        background: 'radial-gradient(ellipse at center, hsl(180 50% 40% / 0.4), transparent 70%)',
        filter: 'blur(100px)'
      }} />
        {/* Right accent */}
        <div className="absolute top-1/2 right-0 -translate-y-1/2 w-[400px] h-[600px] opacity-15" style={{
        background: 'radial-gradient(ellipse at center, hsl(var(--accent) / 0.3), transparent 70%)',
        filter: 'blur(100px)'
      }} />
      </div>
      ...
      {/* Glass Card */}
      <div className="relative z-10 w-full max-w-md px-4 sm:px-0" style={cardStyle}>
        {/* Card border glow */}
        <div className="absolute -inset-[1px] rounded-3xl opacity-60" style={{
        background: 'linear-gradient(145deg, hsl(0 0% 100% / 0.1), hsl(0 0% 100% / 0.02), hsl(0 0% 100% / 0.08))'
      }} />
        
        {/* Lightning effects on edges */}
        <div className="absolute -inset-1 rounded-3xl opacity-40 blur-sm" style={{
        ...
        boxShadow: '0 25px 50px -12px hsl(0 0% 0% / 0.5), inset 0 1px 0 0 hsl(0 0% 100% / 0.05)'
          ...
          <div className="absolute top-0 left-0 right-0 h-32 opacity-30 pointer-events-none" style={{
          ...
          <div className="relative p-5 sm:p-8">
            ...
            <div className="text-center mb-6 sm:mb-8" style={headerStyle}>
              <Link to="/" className="inline-block text-lg sm:text-xl font-bold text-foreground hover:text-accent transition-colors mb-4 sm:mb-6">
                MultiBlock
              </Link>
            ...
            {(mode === 'signin' || mode === 'signup') && <div style={formFieldStyle(0)}>
                <Button type="button" variant="outline" onClick={handleGoogleSignIn} disabled={isGoogleLoading || isLoading} className="w-full h-10 sm:h-12 rounded-xl gap-3 bg-white hover:bg-gray-50 text-gray-700 border-border/50 font-medium text-sm transition-all">
            ...
            {/* Divider */}
            {(mode === 'signin' || mode === 'signup') && <div className="relative py-2" style={formFieldStyle(50)}>
            ...
            <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
              {mode === 'signup' && <div className="space-y-1.5 sm:space-y-2" style={formFieldStyle(0)}>
                  <Label htmlFor="fullName" className="text-xs text-muted-foreground uppercase tracking-wider">
                    Full Name
                  </Label>
                  <Input id="fullName" type="text" placeholder="John Doe" value={fullName} onChange={e => setFullName(e.target.value)} className="h-10 sm:h-12 text-sm sm:text-base bg-secondary/30 border-border/50 rounded-xl focus:border-foreground/50 focus:ring-foreground/20 transition-all" />
              ...
              <div className="space-y-1.5 sm:space-y-2" style={formFieldStyle(mode === 'signup' ? 50 : 0)}>
                ...
                <Input id="email" type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} className="h-10 sm:h-12 text-sm sm:text-base bg-secondary/30 border-border/50 rounded-xl focus:border-foreground/50 focus:ring-foreground/20 transition-all" />
                {errors.email && <p className="text-sm text-destructive">{errors.email}</p>}
              </div>
              
              {mode !== 'forgot-password' && <div className="space-y-1.5 sm:space-y-2" style={formFieldStyle(mode === 'signup' ? 100 : 50)}>
                  <Label htmlFor="password" className="text-xs text-muted-foreground uppercase tracking-wider">
                    Password
                  </Label>
                  <div className="relative">
                    <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="pr-11 h-10 sm:h-12 text-sm sm:text-base bg-secondary/30 border-border/50 rounded-xl focus:border-foreground/50 focus:ring-foreground/20 transition-all" />
              ...
              {mode === 'signin' && <div className="flex justify-end" style={formFieldStyle(100)}>
                  <button type="button" onClick={() => setMode('forgot-password')} className="text-xs text-muted-foreground hover:text-foreground transition-colors">
                    Forgot password?
                  </button>
                </div>}
            ...
            <div className="mt-6 text-center" style={footerStyle}>
              {mode === 'signin' && <p className="text-sm text-muted-foreground">
                  ...
                  <button onClick={() => setMode('signup')} className="text-foreground hover:text-accent font-medium transition-colors">
                    Sign up
                  </button>
                </p>}
              {mode === 'signup' && <p className="text-sm text-muted-foreground">
                  Already have an account?{" "}
                  <button onClick={() => setMode('signin')} className="text-foreground hover:text-accent font-medium transition-colors">
                    Sign in
                  </button>
                </p>}
              {mode === 'forgot-password' && <button onClick={() => setMode('signin')} className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1 mx-auto transition-colors">
